    ...
    public void onActivityResult(int reqcode, int resultcode, Intent data) {
        ...
        if (reqcode == PICK_CONTACT && resultcode == Activity.RESULT_OK) {
            ...
            if (cursor1.moveToFirst()) {
                String id = cursor1.getString(cursor1.getColumnIndexOrThrow(
                        ContactsContract.Contacts._ID));
                String nombre = cursor1.getString(cursor1.getColumnIndexOrThrow(
                        ContactsContract.Contacts.DISPLAY_NAME));
                tvResultado.setText(id + ", " + nombre);
                try {
                    Cursor telefonos = getContentResolver().query(
                    ...
                    for (int i = 0; i < t; i++) {
                        String tel = telefonos.getString(telefonos.getColumnIndexOrThrow(
                                ContactsContract.CommonDataKinds.Phone.DATA));
                        tvResultado.append(tel);
